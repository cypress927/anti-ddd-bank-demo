...
public final class Commands {
    ...
    public record TransferRequest(Long sourceAccountNo, Long destinationAccountNo,
                                   double amount, boolean isInternal) {
        public TransferRequest(Long sourceAccountNo, Long destinationAccountNo, double amount) {
            this(sourceAccountNo, destinationAccountNo, amount, true);
        }
    }
    public record AddAccountManagerRequest(Long accountNo, String username) {}
    public record UpdateRuleRequest(String ruleKey, double value) {}
    public record AccrueInterestRequest(long days) {}

    /** Banker login — demo credentials only. */
    public record BankerLoginRequest(String username, String password) {}
    ...
    public record AccountResponse(Long accountNo, String name, String balance, String accountType) {
        public static AccountResponse from(bank.domain.facts.AccountFact f) {
            return new AccountResponse(f.accountNo().number(), f.name(),
                f.balance().toString(), f.accountType().name());
        }
    }

    public record AccessResponse(String clientUsername, boolean isOwner,
                                  Long accountNo, String accountName,
                                  String accountBalance, String accountType) {
        public static AccessResponse from(bank.domain.facts.AccessFact f) {
            return new AccessResponse(f.clientUsername(), f.isOwner(),
                f.accountNo().number(), f.accountName(),
                f.accountBalance().toString(), f.accountType().name());
        }
    }
}
